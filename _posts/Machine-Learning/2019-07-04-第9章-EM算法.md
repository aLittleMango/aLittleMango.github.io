...
$$
= \color {green} \log \color {black} \left( \sum _ { Z } \left[ \color {blue} P \left( Z  \mid  Y , \theta ^ { ( i ) } \right) \color {red} \frac { P ( Y  \mid  Z , \theta ) P ( Z  \mid  \theta ) } { P \left( Z  \mid  Y , \theta ^ { ( i ) } \right) } \right] \right) - \log P \left( Y  \mid  \theta ^ { ( i ) } \right) \quad \text {（9.2）}
$$
...
$$
\color {green} f \color {black} \left( \sum _ {j} \left[ \color{blue}  \lambda _ {j} \color{red} y _ {j} \right] \right) \geq \sum _ {j} \left[ \color{blue} \lambda _ {j} \color{black} \color {green} f \color {black} \left( \color{red} y _ {j} \right) \right] \qquad \text{其中 } \lambda _ {j} \geq 0 , \color {orange} \sum _ {j} \color{blue} \lambda _ {j} \color{black} = 1
$$

- **9.3 → 9.4**

$$
\color{orange} \sum_Z \left[ \color{blue} P \left(Z \mid Y,\theta^{\left( i \right) } \right) \right] \color{black} = 1
$$
