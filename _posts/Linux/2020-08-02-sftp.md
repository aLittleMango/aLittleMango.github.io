---
layout: post
title: Ubuntu 通过 sftp 远程登录 linux 系统
category: Linux
tags: Linux
keywords: 内核
description:
...
```bash
sftp xieqi@100.130.100.99   # 以 xieqi 用户身份远程登录100.130.100.99主机，默认端口号为22
sftp -o Port=222 xieqi@100.130.100.99   # 指定 SSH 端口号来远程登录主机
sftp -o Port=222 xieqi@100.130.100.99:/home/xieqi/local/    # 指定 SSH 端口号来远程登录主机，并切换到 /home/xieqi/local/ 目录
sftp -P 222 xieqi@100.130.100.99    # 指定 SSH 端口号来远程登录主机（此方式只能在 Ubuntu 10.10 中使用）
```

### 上传

```bash
put new.txt /home/xieqi/  # 将本地当前目录下的 new.txt 上传到远程主机的 /home/xieqi/ 目录
```
...
!                           # 切换到本地 shell，以便执行其他命令，如果想退出本地 shell，可以执行 exit 以返回远程环境
```
